...
class VKT(Stat):
    """
    Compute V @ K^T where V is output and K is input.
    Input format: V^T (d_v, n) and K^T (d_k, n), where d is feature dimension, n is number of samples.
    Similar to SecondMoment, computes the cross-covariance matrix directly.
    
    This computes E[(V_i - mean_V) @ (K_i - mean_K)^T] (centered version).
    Result shape: (d_v, d_k)
    """
    ...
    def __init__(self, split_batch=True, state=None):
        ...
        self.count = 0
        self._mean = None
        self.mom2 = None  # Sum of V @ K^T for all samples
        self.split_batch = split_batch
    
    def add(self, v, k):
        """
        Add a batch of V (output) and K (input) samples.
        Input format: 
        - v: (n, d_v) - V matrix, where n is number of samples, d_v is feature dimension
        - k: (n, d_k) - K matrix, where n is number of samples, d_k is feature dimension
        
        Computes V @ K^T = sum_i (V_i @ K_i^T) where V_i and K_i are row vectors.
        For each sample i: V_i @ K_i^T (outer product) = (d_v, d_k)
        Sum over all samples gives (d_v, d_k)
        
        Result shape: (d_v, d_k)
        """
        # Handle shape normalization - ensure 2D
        if len(v.shape) == 1:
            v = v.unsqueeze(0)  # (d_v,) -> (1, d_v)
        if len(k.shape) == 1:
            k = k.unsqueeze(0)  # (d_k,) -> (1, d_k)
        ...
        # Now v is (n, d_v) and k is (n, d_k)
        assert v.shape[0] == k.shape[0], f"V and K must have same number of samples: {v.shape[0]} vs {k.shape[0]}"
        
        batch_count = v.shape[0]  # n samples
        
        # Transpose to (d, n) format for computation: V^T (d_v, n) and K^T (d_k, n)
        v_t = v.T  # (n, d_v) -> (d_v, n)
        k_t = k.T  # (n, d_k) -> (d_k, n)
        ...
        if self._mean is None:
            self.count = batch_count
            # Mean of V and K: sum over samples (dimension 1 of transposed), then divide by n
            self._mean = [v_t.sum(1) / batch_count, k_t.sum(1) / batch_count]  # (d_v,), (d_k,)
            # Center: subtract mean
            centered_v_t = v_t - self._mean[0].unsqueeze(1)  # (d_v, n)
            centered_k_t = k_t - self._mean[1].unsqueeze(1)  # (d_k, n)
            
            # Compute V @ K^T = sum_i (V_i @ K_i^T)
            # V_i is the i-th row of V: V[i, :] shape (d_v,)
            # K_i is the i-th row of K: K[i, :] shape (d_k,)
            # V_i @ K_i^T = V[i, :].unsqueeze(-1) @ K[i, :].unsqueeze(0) = (d_v, 1) @ (1, d_k) = (d_v, d_k)
            # Sum over all samples: centered_v_t @ centered_k_t.T = (d_v, n) @ (n, d_k) = (d_v, d_k)
            self.mom2 = centered_v_t @ centered_k_t.T  # (d_v, n) @ (n, d_k) = (d_v, d_k)
            return
        
        # Update using Chan-style update for numerical stability
        old_count = self.count
        self.count += batch_count
        
        # Update means incrementally
        self._mean[0].add_((v_t.sum(1) - self._mean[0] * old_count) / self.count)
        self._mean[1].add_((k_t.sum(1) - self._mean[1] * old_count) / self.count)
        
        # Recompute centered values with updated mean
        centered_v_t = v_t - self._mean[0].unsqueeze(1)  # (d_v, n)
        centered_k_t = k_t - self._mean[1].unsqueeze(1)  # (d_k, n)
        
        # Update mom2: add V @ K^T for this batch
        # centered_v_t @ centered_k_t.T = (d_v, n) @ (n, d_k) = (d_v, d_k)
        batch_mom2 = centered_v_t @ centered_k_t.T  # (d_v, d_k)
        self.mom2 = self.mom2 + batch_mom2
    
    def mean(self):
        """Return the mean of V and K."""
        return self._mean
    
    def moment(self, unbiased=True):
        """
        Return E[(V_i - mean_V) @ (K_i - mean_K)^T] (centered cross-covariance).
        Shape: (d_v, d_k)
        """
        if self.mom2 is None:
            return None
        return self.mom2 / (self.count - (1 if unbiased else 0))
    ...
    def state_dict(self):
        ...
        return dict(
            constructor=self.__module__ + "." + self.__class__.__name__ + "()",
            count=self.count,
            mean_v=self._mean[0].cpu().numpy() if self._mean is not None else None,
            mean_k=self._mean[1].cpu().numpy() if self._mean is not None else None,
    ...
    def load_state_dict(self, state):
        ...
        self.count = int(state["count"])
        if state["mean_v"] is not None:
            self._mean = [
                torch.from_numpy(state["mean_v"]),
                torch.from_numpy(state["mean_k"])
            ]
        if state["mom2"] is not None:
            self.mom2 = torch.from_numpy(state["mom2"])
